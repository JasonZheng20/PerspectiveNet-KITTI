...
from umautobots_converter.converter import Ingestor, Egestor
...
class KITTIIngestor(Ingestor):
    ...
    def validate(self, path):
        ...
        if not os.path.isfile(f"{path}/%s.txt"%(self.split)):
            return False, f"Expected %s.txt file within {path}" % (self.split)
        return True, None
